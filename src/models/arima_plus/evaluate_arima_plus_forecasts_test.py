...
"""Tests for evaluate_arima_plus_forecasts."""
...
from models.arima_plus import evaluate_arima_plus_forecasts
from common import utils
from google3.testing.pybase import googletest


class EvaluateArimaPlusForecastsTest(googletest.TestCase):
  ...
  def test_calculate_metrics_success(self):
    ...
    metrics = evaluate_arima_plus_forecasts.calculate_metrics(
        actuals, forecasts
    )
  ...
  def test_calculate_metrics_zero_actual_sum(self):
    ...
    with self.assertRaises(ValueError):
      evaluate_arima_plus_forecasts.calculate_metrics(actuals, forecasts)

  def test_generate_evaluation_plot_success(self):
    plot_data = pd.DataFrame({
        "date": pd.to_datetime(["2026-01-01", "2026-02-01"]),
        "actual": [100.0, 200.0],
        "arp_forecast": [110.0, 190.0],
        "arp_lower_bound": [90.0, 180.0],
        "arp_upper_bound": [120.0, 210.0],
    })
    p = evaluate_arima_plus_forecasts.generate_evaluation_plot(
  ...
  @mock.patch.object(
      evaluate_arima_plus_forecasts.gg.ggplot, "save", autospec=True
  )
  def test_evaluate_arp_forecast_precision_success(
    ...
    self.fs.create_file(
        "data/forecast/arima_plus/test_tag_forecast.csv",
        contents=(
    ...
    evaluate_arima_plus_forecasts.evaluate_arp_forecast_precision_and_plot(
        "params.yaml"
    )
    ...
    self.assertTrue(
        os.path.exists("data/evaluate/arima_plus/test_tag_metrics.json")
    )

  @mock.patch.object(utils, "upload_to_gcs", autospec=True)
  def test_evaluate_arp_forecast_precision_missing_prerequisites(
      self, mock_upload
  ):
    self.fs.create_file(
        "params.yaml",
