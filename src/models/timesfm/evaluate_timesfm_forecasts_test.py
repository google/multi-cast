...
"""Tests for evaluate_timesfm_forecasts."""
...
from models.timesfm import evaluate_timesfm_forecasts
from common import utils
from google3.testing.pybase import googletest


class EvaluateTimesfmForecastsTest(googletest.TestCase):
  ...
  def test_calculate_metrics_success(self):
    ...
    metrics = evaluate_timesfm_forecasts.calculate_metrics(actuals, forecasts)
  ...
  def test_calculate_metrics_zero_actual_sum(self):
    ...
    with self.assertRaises(ValueError):
      evaluate_timesfm_forecasts.calculate_metrics(actuals, forecasts)

  def test_generate_ribbon_plot_success(self):
    plot_data = pd.DataFrame({
        "date": pd.to_datetime(["2026-01-01", "2026-02-01"]),
        "actual": [100.0, 200.0],
        "tfm_forecast": [110.0, 190.0],
        "tfm_lower_bound": [90.0, 180.0],
        "tfm_upper_bound": [120.0, 210.0],
    })
    p = evaluate_timesfm_forecasts.generate_ribbon_plot(
  ...
  @mock.patch.object(
      evaluate_timesfm_forecasts.gg.ggplot, "save", autospec=True
  )
  def test_evaluate_tfm_forecast_precision_success(
      self, mock_save, mock_upload
  ):
    os.makedirs("data/split", exist_ok=True)
    os.makedirs("data/forecast/timesfm", exist_ok=True)
    self.fs.create_file(
        "params.yaml",
        contents=(
            "tag: test_tag\n"
    ...
    self.fs.create_file(
        "data/forecast/timesfm/test_tag_forecast.csv",
        contents=(
    ...
    evaluate_timesfm_forecasts.evaluate_tfm_forecast_precision_and_plot(
        "params.yaml"
    )
    ...
    self.assertTrue(
        os.path.exists("data/evaluate/timesfm/test_tag_metrics.json")
    )

  @mock.patch.object(utils, "upload_to_gcs", autospec=True)
  def test_evaluate_tfm_forecast_missing_prerequisites(self, mock_upload):
    self.fs.create_file(
        "params.yaml",
