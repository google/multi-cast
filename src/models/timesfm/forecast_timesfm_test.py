...
from models.timesfm import forecast_timesfm
from google3.testing.pybase import googletest


class ForecastTimesfmTest(googletest.TestCase):
  ...
  def test_build_timesfm_query_success(self):
    query = forecast_timesfm.build_timesfm_query(
  ...
  def test_execute_timesfm_forecast_success(self, mock_client_cls, mock_upload):
    ...
    forecast_timesfm.execute_timesfm_forecast("params.yaml")

    mock_client.query.assert_called_once()
    mock_upload.assert_called_once()
    self.assertTrue(
        os.path.exists("data/forecast/timesfm/test_tag_forecast.csv")
    )

  @mock.patch.object(utils, "upload_to_gcs", autospec=True)
  def test_execute_timesfm_forecast_missing_params(self, mock_upload):
  ...
  @mock.patch.object(utils, "upload_to_gcs", autospec=True)
  def test_execute_timesfm_forecast_missing_source(self, mock_upload):
    self.fs.create_file(
        "params.yaml",
        contents=(
            "tag: test_tag\n"
